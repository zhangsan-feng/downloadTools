...
async function DownLoadForHtml(source_data){}



async function DownLoadForApi(source_data, cookie){
    const music_id = source_data.download_link.split("?")[0].replace("https://www.kuwo.cn/play_detail/","")
    ...
    if (response_body["msg"] ==="success"){
        ...
        const resource_params:ResourceParams = {
            ...
            platform:"kuwo",
            req_headers:request_headers,
            download_link: { [source_data.file_name]: mp3_link }
        }
...
export async function KuWoMusicDetails(source_data){
    const cookie = ""
    await DownLoadForHtml(source_data)
    await DownLoadForApi(source_data, cookie)
}
